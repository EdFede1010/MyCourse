...
namespace MyCourses.Models.Services.Application.Lessons
{
    public class AdoNetLessonService : ILessonService
    {
        ...
        {
            FormattableString query = $@"SELECT Id, Title, Description, Duration, RowVersion, [Order], CourseId FROM Lessons WHERE ID={id}";
        ...
        {
            int affectedRows = await db.CommandAsync($"DELETE FROM dbo.Lessons WHERE Id = {inputModel.Id};");
            if (affectedRows == 0)
            {
                throw new LessonNotFoundException(inputModel.Id);
            }
